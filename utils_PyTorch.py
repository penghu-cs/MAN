...
def myLog(x, threshold=100):
    # return (-F.threshold(-x, -threshold, -threshold)).log()
    return x.log()
...
def fisher_loss(data, labels, num_classes=10, eta=1e-3):
    ...
    for c in classes:
        ...
        tmp = m - mi
        tmp = myActive(tmp)
        # Sb += F.tanh(tmp.t().mm(tmp)) * Xi.shape[0]
        Sb += tmp.t().mm(tmp) * Xi.shape[0]
        # Sb += F.tanh(tmp.t().mm(tmp) * Xi.shape[0])
        # Sb += myActive(tmp.t().mm(tmp)) * Xi.shape[0]
    ...
    # top_k_evals = torch.log(top_k_evals)
    ...
    # costs = (-(1e2 - top_k_evals).pow(3) * top_k_evals.log()).mean()
    # costs = (-(1. / top_k_evals).pow(0.1) * top_k_evals.log()).mean()
    # costs = (-top_k_evals.log()).mean()
    costs = (-myLog(top_k_evals)).mean()
    return costs

def myActive(x):
    # return (x >= 0).float() * (x + 1.).abs().log() - (x < 0).float() * (1. - x).abs().log()
